...
* What threshold ``delta2`` makes pool 2 deliver the same consensus
  quality as pool 1 running at ``delta1``?  (Theorem 6.1)
...
def _require_same_side(phi1: np.ndarray, phi2: np.ndarray) -> None:
    """Both pools must sit on the same side of chance (Theorem 6.1 hypothesis)."""
...
# ---------------------------------------------------------------------------
# Theorem 6.1 — quality-equivalent threshold
# ---------------------------------------------------------------------------

def equivalent_delta(phi1: ArrayLike, delta1: ArrayLike, phi2: ArrayLike) -> np.ndarray:
    """Threshold for pool 2 that matches pool 1's quality (Theorem 6.1, §6.1).

    δ₂ = δ₁ · ln(φ₁) / ln(φ₂)

    Setting ``Q(phi1, delta1) == Q(phi2, delta2)`` (Theorem 4.1) forces
...
# ---------------------------------------------------------------------------
# Theorem 6.2 — cost-equivalent payment
# ---------------------------------------------------------------------------

def equivalent_payment(phi: ArrayLike) -> np.ndarray:
    """Relative per-vote payment for a pool with odds ``phi`` (Theorem 6.2, §6.2).
    ...
    value. For two quality-matched pools, Theorem 6.2 gives
...
def quality_matched_pools(
    ...
    """Matched threshold for each pool in ``phi_list`` (Theorem 6.1, §6.1).
